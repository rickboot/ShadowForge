...
import { WireframeGeometry, LineSegments, LineBasicMaterial, IcosahedronGeometry, Group } from 'three';
...
function createD20Wireframe(radius: number = 0.5, scale: number = 1): LineSegments {
  ...
  const material = new LineBasicMaterial({ color: 0x02816F, opacity: 0.5, transparent: true });
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      ...
      if (wireD20Ref.current) {
        wireD20Ref.current.rotation.x = Math.sin(time) * 1;
        wireD20Ref.current.rotation.z = Math.cos(time) * 1;
      }
    }
  });
